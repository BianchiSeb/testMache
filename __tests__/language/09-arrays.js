describe("Arrays", () => {
  ...
  it("Arrays - 5", () => {
    ...
    var odd = collection.filter(nb => {return nb % 2 === 1});
    expect(odd).toEqual([1, 3, 5, 7, 9]);
  });

  it("Arrays - 6", () => {
    var collection = [1, 2, 3, 4];
    var result = collection.reduce((accu, curr) => {return accu * curr});
    expect(result).toBe(24);
  });

  it("Arrays - 7", () => {
    var collection = [1, 2, 3];
    expect(collection.map(nb => {return nb * 2})).toEqual([2, 4, 6]);
  });
  ...
  it("Arrays - 8", () => {
    ...
    expect([array1.concat(array2)].flat()).toEqual([1, 2, 3, 4, 5, 6]);
  });
